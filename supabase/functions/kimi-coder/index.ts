/** @doc Kimi 2.6 agentic coder — plan→act→observe loop with parallel tool calls (write_file, edit_file, read_file, bash, python, todo_write). SSE stream. */
...
// ---------- System prompt (Kimi-style agent) ----------
const SYSTEM = `You are Kimi Coder, an autonomous full-stack coding agent by Megsy.
NEVER mention providers (Moonshot, Alibaba, Qwen, OpenAI, Anthropic, Google). You are Megsy.

WORKFLOW:
1. Analyze the request briefly (1-2 sentences).
2. Call \`todo_write\` with a numbered plan of concrete build steps.
3. For each step: issue MULTIPLE tool calls IN PARALLEL in the SAME assistant message (batch write_file/edit_file together). Never write files one by one when you can batch.
4. After batches, call \`bash\` for install/build/test. Observe output. If errors, use \`read_file\`+\`edit_file\` to repair, then re-run.
5. Update \`todo_write\` as steps complete (mark done: true).
6. Finish with a short summary (files created, how to run).

RULES:
- Stack default: Vite + React + TypeScript + Tailwind + shadcn/ui + Supabase. Full production code, no TODOs, no placeholders.
- Prefer PARALLEL tool calls — batch 5-15 writes per message.
- Paths are relative (e.g. "src/App.tsx", "package.json").
- Always create package.json, index.html, vite.config.ts, tsconfig.json, tailwind.config.ts, src/main.tsx, src/App.tsx, README.md at minimum.
...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: "edit_file",
      description: "Replace an exact string in an existing file (str_replace).",
      parameters: {
        type: "object",
        properties: {
          path: { type: "string" },
  ...
  },
  {
    ...
    function: {
      name: "bash",
      description: "Simulated shell command (npm install/build/test). Returns emulated output — do not rely on filesystem side-effects; use write_file to create files.",
      parameters: {
        type: "object",
...
function execTool(
  ...
  try {
    ...
    if (name === "bash") {
      ...
      // Simulated — no real fs. Report success for common commands.
      ...
      if (/npm run build|vite build/i.test(cmd)) {
        // Check for at least an entry file
...
async function callModel(
  ...
  if (!resp.ok) {
    ...
    return { message: null, error: `model error ${resp.status}: ${t.slice(0, 300)}` };
  }
...
Deno.serve(async (req) => {
  ...
  // Pick Kimi 2.6 when available on the provider; otherwise use provider's coder model.
  const model =
    llm.provider === "openrouter"
      ? "moonshotai/kimi-k2"
      : llm.mapModel("qwen3-coder-plus");
    ...
    async start(controller) {
      ...
      try {
        ...
        for (let step = 0; step < MAX_STEPS; step++) {
          const { message, error } = await callModel(llm.url, llm.key, model, messages);
          if (error) {
          ...
          for (const tc of toolCalls) {
            ...
            // Broadcast side-effects to the client UI.
